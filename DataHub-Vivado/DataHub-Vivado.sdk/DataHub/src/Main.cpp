    ...
    while (1)
    {
        switch (state)
        {
			...
			case SENSOR_REG_PREP:
				...
				xil_printf("Preparing to register %d sensors...\r\n", numSensors);
			...
			case SEND_SENSOR_REG:
				if (numSensorsRegistered < numSensors)
				{
					...
					xil_printf("Registering %s...\r\n", sensorInfoCollection[numSensorsRegistered].sensorName);
					ChangeStatePrintTransition(SEND);
					break;
				}
				ChangeStatePrintTransition(CLOSE);
			...
			case PUBLISH_SENSOR_DATA:
				...
				CreateSensorDataPubMsg(&msg2Send, (uint8_t*)&sensorDataPayload);
				ChangeStatePrintTransition(SEND);
				break;
			...
			case SEND:
				...
				{
					PrintMsgTypeAndDest((MsgTypes)msg2Send.header.msgType, HUB);
			...
			case CLOSE:
				...
				xil_printf("Sequence complete, the device will now idle!\r\n");
				state = DONE;
				break;
